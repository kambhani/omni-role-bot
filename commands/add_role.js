...
const execute = async (interaction) => {
  ...
  try {
    ...
    await interaction.reply(
      `${userList.length} member(s) were requested the role addition "${role.name}" by ${interaction.user.username}. Of the member(s) requested, ${assigned} role update(s) happened successfully.`
    );
  } catch (err) {
    console.log(err);
